    ...
    for (int i = 0; i < MAPWIDTH; ++i) {
        for (int k = 0; k < 4; ++k) {
            if (t.isValid(curState, i, MAPHEIGHT - 1, k)) {
                can[MAPHEIGHT - 1][i] |= (1 << k);
            }
        }
    }
    
    for (int j = MAPHEIGHT - 2; j > 0; --j) {
        memcpy(can[j], can[j+1], sizeof can[j]);
